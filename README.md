...
Time Complexity: O(n^2), because we use a nested loop, an if inside an if. So the execution time grows quadratically with the size of the input.
Space Complexity: O(1), we are not using extra data structures to store values during the search.
